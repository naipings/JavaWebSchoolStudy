...
router.get('/projectList', (req, res) => {
    ...
    const sqlLen = "select * from book";
    sqlFn(sqlLen, null, data => {
        let len = data.length;
        const sql = "select * from book order by id desc limit 8 offset " + (page - 1) * 8;
...
router.get("/search", (req, res) => {
    ...
    const sql = "select * from book where concat(`title`,`sellPoint`,`descs`) like '%" + search + "%'";
    sqlFn(sql, null, (result) => {
        if (result.length > 0) {
            res.send({
                status: 200,
